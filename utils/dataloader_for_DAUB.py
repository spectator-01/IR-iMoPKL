...
class seqDataset(Dataset):
    def __init__(self, dataset_path, image_size, num_frame=5 ,type='train'):
        ...
        ###############################
        description = pickle.load(open('mediate_data/emb_train_DAUB.pkl', 'rb'))
        ...
        relation = pickle.load(open('mediate_data/motion_relation_DAUB.pkl', 'rb'))
        ...
        # TODO test
        # 检查 annotation 中的图像文件是否存在
        for p in self.img_idx:
            if not os.path.isfile(p):
                raise FileNotFoundError(f"annotation contains nonexistent image '{p}'")
        # …
    ...
    def get_caption_data(self, index):
        ...
        # print("self.cap_idx:", self.cap_idx)
        # print("file_name:", file_name)   right
        #
        caption_frames = []
        relation = []
    ...
    def get_boxes_data(self, index):
        ...
        #     image_path = os.path.join(image_dir, f"{frame_id}.bmp")
